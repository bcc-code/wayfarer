...
<template>
  <UFormField :ui="{ label: 'text-label' }" :label>
    <UTextarea
      v-model="modelValue"
      :ui="{
        base: 'bg-background-indent rounded-list! focus-visible:ring-accent ring-border-default aria-[invalid=true]:ring-accent-negative text-label p-medium! placeholder:text-text-hint',
      }"
      :rows="8"
      :placeholder
      :disabled
