...
} = useMaintenanceScoreJournalPreviewQuery({
...
const { executeMutation: fix, fetching: fixing } =
  useFixMissingScoreJournalAsyncMutation()
...
async function handleFix() {
  ...
  const result = await fix({ achievementId: ACHIEVEMENT_ID })
  if (result.data?.fixMissingScoreJournalAsync) {
    const jobs = result.data.fixMissingScoreJournalAsync
    if (jobs.length > 0) {
      jobId.value = jobs[0].id
      jobStatus.value = jobs[0].status
  ...
  <UContainer class="py-12">
    <div class="mb-8">
      <div class="mb-4 flex items-center gap-2">
        ...
        />
        <h1 class="text-3xl">Fix poengjournal</h1>
      </div>
      <p class="text-muted">
        ...
        Det oppretter manglende oppforinger slik at poengene blir registrert.
      </p>
      ...
      >
        <div class="flex flex-col gap-2">
          ...
          <ul class="text-muted list-inside list-disc text-sm">
            <li>{{ jobResult.successCount }} oppforinger opprettet</li>
    ...
    <UModal v-model:open="showConfirmModal">
      <template #content>
        <UCard>
          ...
          <p class="text-muted">
            Er du sikker på at du vil kjore reparasjonen? Dette vil opprette
            {{ preview?.totalEvents ?? 0 }} manglende poengjournal-oppforinger
            for {{ preview?.totalUsers ?? 0 }} brukere.
          </p>
          <template #footer>
