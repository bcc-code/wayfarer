...
} = useMaintenanceStreakProgressPreviewQuery({
  pause: computed(() => !isAuthReady.value),
})

const preview = computed(() => data.value?.previewMissingStreakProgress)
const affectedUsers = computed(() => preview.value?.affectedUsers ?? [])

type UserRow = NonNullable<
  MaintenanceStreakProgressPreviewQuery['previewMissingStreakProgress']
...
const { executeMutation: fix, fetching: fixing } =
  useFixMissingStreakProgressAsyncMutation()
  ...
  <UContainer class="py-12">
    <div class="mb-8">
      <div class="mb-4 flex items-center gap-2">
        ...
        />
        <h1 class="text-3xl">Fix streakfremgang</h1>
      </div>
      <p class="text-muted">
        Dette verktøyet finner brukere som har fullført eksterne
        innholdsoppgaver for streak-prestasjoner, men som mangler tilhørende
        fremgangsregistreringer. Kun hendelser der innholdet ble fullført før
        fristen tas med. Det oppretter manglende registreringer og tildeler
        prestasjoner der alle krav er oppfylt.
      </p>
    </div>
