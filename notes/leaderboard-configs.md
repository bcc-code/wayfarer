...
- **Table**: `leaderboard_configs`
  ...
  - `max_entries INT` — optional positive cap on the main list (migration 00104)
...
`maxEntries: Int` is an optional positive integer on `LeaderboardConfig` and
both create/update inputs. Migration `00104_add_leaderboard_config_max_entries.sql`
adds nullable `max_entries INT CHECK (max_entries > 0)`. Existing configs remain
uncapped. A full-replace update with `maxEntries: null` clears the limit.

The limit caps the **number of main-list entries**, for all entity types and both
project/event scopes. The config resolver asks the shared leaderboard service for
the top N entries, then applies cursor pagination within that list. A large
`first`, an `after` cursor beyond the cap, or backward pagination cannot expose
entries outside it. Ties do not expand the configured row count. Rank-based
cursors retain their existing limitation when multiple entries share a rank.
`totalCount` still counts all matching entries, and `me` / `nearestChurchRivals`
remain independent of the main-list cap. The full-board cache is unchanged and
can be shared by configs with different limits.

When neither `first` nor `last` is specified, a configured board returns its
`maxEntries`, or a default page of 100 when no cap is configured. Clients can
request smaller pages and use `pageInfo` within the capped list. The standings
page omits a page size so each config controls its own limit, including values
above 100. The admin form can set or clear the limit; reordering preserves it.

The old participant-count-based caps (3/10/20) remain removed. Ad-hoc
`Project.leaderboard` / `Event.leaderboard` continue using their existing
pagination and do not inherit any config's limit.

## Reused leaderboard engine

`LeaderboardConfig.leaderboard` does **not** reimplement leaderboard computation — it adapts a config into the existing `services.LeaderboardParams` (see `backend/internal/graph/api/leaderboards.go:buildLeaderboardParamsFromConfig`) and calls the same `LeaderboardService.GetProjectLeaderboard`/`GetEventLeaderboard` used by the ad-hoc fields, then config-scoped pagination and the same `buildLeaderboardConnection` helper (which takes `rivalsContextID`/`rivalsIsEvent`/`rivalsFilter` and computes `nearestChurchRivals` lazily — see `feat/nearest-neighboor-ranking`'s "avoid computing rivals on every leaderboard query" refactor, which this branch rebased onto). This means:
- Caching and `nearestChurchRivals` use the same engine for configured and ad-hoc boards. Config pagination uses rank-based cursors within the capped list.
