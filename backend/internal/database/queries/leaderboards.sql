...
WITH project_users AS MATERIALIZED (
    ...
    WHERE up.project_id = @projectid::char(28)
      ...
      AND (@teamid::char(28) = '' OR EXISTS (
          ...
          WHERE tm.user_id = u.id AND tm.team_id = @teamid::char(28)
      ))
      AND (@superteamid::char(28) = '' OR EXISTS (
          SELECT 1 FROM team_members tm
          INNER JOIN teams t ON tm.team_id = t.id
          WHERE tm.user_id = u.id AND t.super_team_id = @superteamid::char(28)
      ))
),
...
person_scores AS (
    ...
    LEFT JOIN score_journal sj ON sj.user_id = fu.id AND sj.project_id = @projectid::char(28)
...
WITH ranked_scores AS (
    ...
    INNER JOIN churches c ON u.church_id = c.id
    WHERE lpp.project_id = @projectid::char(28)
...
SELECT entity_id, name, church_name, image, score, rank, last_score_at
FROM ranked_scores
WHERE entity_id = @userid::char(28);
...
WITH ranked_scores AS (
    ...
    WHERE lpp.project_id = @projectid::char(28)
      ...
      AND (
          @teamid::char(28) != '' OR @superteamid::char(28) != ''
          OR EXISTS (
...
WITH team_scores AS (
    ...
    WHERE
        t.project_id = @projectid::char(28)
        ...
        AND (@churchid::char(28) = '' OR EXISTS (
            SELECT 1 FROM team_members tm2
            INNER JOIN users u2 ON tm2.user_id = u2.id
            WHERE tm2.team_id = t.id AND u2.church_id = @churchid::char(28)
        ))
...
WITH ranked_scores AS (
    ...
    WHERE lpt.project_id = @projectid::char(28)
      AND t.leaderboard_excluded = false
      ...
      AND (@churchid::char(28) = '' OR EXISTS (
          SELECT 1 FROM team_members tm
          INNER JOIN users u ON tm.user_id = u.id
          WHERE tm.team_id = t.id AND u.church_id = @churchid::char(28)
      ))
),
user_team AS (
    SELECT team_id
    ...
    WHERE user_id = @userid::char(28)
      AND team_id IN (SELECT id FROM teams WHERE project_id = @projectid::char(28))
    LIMIT 1
)
...
WITH superteam_scores AS (
    ...
    LEFT JOIN score_journal sj ON sj.user_id = u.id AND sj.project_id = @projectid::char(28)
    WHERE
        st.project_id = @projectid::char(28)
...
WITH ranked_scores AS (
    ...
    WHERE lps.project_id = @projectid::char(28)
...
WITH ranked_scores AS (
    ...
    WHERE lpst.project_id = @projectid::char(28)
...
FROM super_teams st
WHERE st.project_id = @projectid::char(28);
...
WITH church_scores AS (
    ...
    LEFT JOIN score_journal sj ON sj.user_id = u.id AND sj.project_id = @projectid::char(28)
    WHERE
        up.project_id = @projectid::char(28)
...
WITH ranked_scores AS (
    ...
    WHERE lpc.project_id = @projectid::char(28)
...
WITH person_scores AS (
    ...
    INNER JOIN churches c ON u.church_id = c.id
    LEFT JOIN score_journal sj ON sj.user_id = u.id AND sj.event_id = @eventid::char(28)
    WHERE
        ue.event_id = @eventid::char(28)
        AND (@churchid::char(28) = '' OR u.church_id = @churchid::char(28))
        AND (@country::text = '' OR EXISTS (
...
WITH ranked_scores AS (
    ...
    INNER JOIN churches c ON u.church_id = c.id
    WHERE lep.event_id = @eventid::char(28)
...
WITH event_project AS (
    SELECT project_id FROM events WHERE id = @eventid::char(28)
),
team_scores AS (
    SELECT
        t.id AS entity_id,
    ...
    INNER JOIN team_members tm ON t.id = tm.team_id
    INNER JOIN users u ON tm.user_id = u.id
    INNER JOIN user_events ue ON u.id = ue.user_id AND ue.event_id = @eventid::char(28)
    LEFT JOIN score_journal sj ON sj.user_id = u.id AND sj.event_id = @eventid::char(28)
    WHERE
...
WITH ranked_scores AS (
    ...
    WHERE let.event_id = @eventid::char(28)
      AND t.leaderboard_excluded = false
...
user_team AS (
    ...
    FROM team_members tm
    INNER JOIN teams t ON tm.team_id = t.id
    INNER JOIN events e ON t.project_id = e.project_id AND e.id = @eventid::char(28)
    WHERE tm.user_id = @userid::char(28)
    LIMIT 1
)
SELECT rs.entity_id, rs.name, rs.image, rs.score, rs.rank, rs.last_score_at
...
WITH ranked_scores AS (
    ...
    WHERE les.event_id = @eventid::char(28)
...
WITH ranked_scores AS (
    ...
    WHERE lest.event_id = @eventid::char(28)
...
WITH ranked_scores AS (
    ...
    WHERE lec.event_id = @eventid::char(28)
