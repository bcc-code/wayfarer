...
-- Add webhook event type for quiz_finalized
...
ALTER TABLE webhooks ADD CONSTRAINT webhooks_event_type_check
    CHECK (event_type IN ('external_content_event', 'points_awarded', 'quiz_session_finished', 'team_name_changed', 'quiz_finalized'));
...
-- Remove quiz_finalized from webhook event types
ALTER TABLE webhooks DROP CONSTRAINT IF EXISTS webhooks_event_type_check;
ALTER TABLE webhooks ADD CONSTRAINT webhooks_event_type_check
