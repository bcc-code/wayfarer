...
// FixMissingScoreJournal creates missing score journal entries for content achievements.
// This finds users who have completed external content but don't have corresponding score_journal entries.
func (s *Service) FixMissingScoreJournal(ctx context.Context, params pubsub.FixMissingScoreJournalParams) (int, int, error) {
	if len(params.UserIDs) == 0 {
		return 0, 0, nil
	}

	// Get the achievement to retrieve project_id, event_id, and points
	achievementThunk := s.Loaders.AchievementByIDLoader.Load(ctx, params.AchievementID)
	achievement, err := achievementThunk()
	if err != nil {
		return 0, 0, fmt.Errorf("failed to load achievement: %w", err)
	}

	// Extract project_id, event_id, and points from the achievement
	...
	case *model.ContentAchievement:
		...
		eventID = a.EventID
		points = int32(a.Points)
	default:
		return 0, 0, fmt.Errorf("achievement %s is not a content achievement", params.AchievementID)
	}

	// Get missing score journal entries for the specific users in this batch
	events, err := s.DB.Queries.GetMissingScoreJournalForUsers(ctx, sqlc.GetMissingScoreJournalForUsersParams{
		Achievementid: params.AchievementID,
		Userids:       params.UserIDs,
	})
	if err != nil {
		return 0, 0, fmt.Errorf("failed to get missing score journal entries: %w", err)
	}
	...
	s.Logger.Info("FixMissingScoreJournal: processing missing score journal entries",
	...
	// Process each event using the standard CreateScoreJournalEntry query
	for _, event := range events {
		...
		_, err := s.DB.Queries.CreateScoreJournalEntry(ctx, sqlc.CreateScoreJournalEntryParams{
			...
			SourceID:   &event.ExternalContentID,
		})
		if err != nil {
			s.Logger.Error("Failed to create score journal entry",
				"user_id", event.UserID,
				"external_content_id", event.ExternalContentID,
				"achievement_id", params.AchievementID,
				"error", err,
			)
			failureCount++
		...
		successCount++
		s.Cache.InvalidateUser(event.UserID)
	}
	...
	s.Logger.Info("FixMissingScoreJournal: completed",
		"achievement_id", params.AchievementID,
		"success_count", successCount,
		"failure_count", failureCount)
