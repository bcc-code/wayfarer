...
// NewClient creates a new Phrase client with configurable target languages
func NewClient(queries *sqlc.Queries, config Config, languages []string) *Client {
...
func (m *Client) SendToTranslation(ctx context.Context, collection string, data []common.TranslationData) error {
	...
	if err != nil {
		log.Error().Err(err).Msg("Unable to marshal output data")
		return err
	}
	...
	if err != nil {
		log.Error().Str("filename", filename).Err(err).Msg("Unable to get jobs")
		return err
	}
	...
	if len(nonExisting) > 0 {
		...
		if err != nil {
			log.Error().Err(err).Msg("Failed to create jobs in Phrase")
			errList = append(errList, err)
		}
	}
	...
	if len(jobs) > 0 {
		...
		if err != nil {
			log.Error().Err(err).Msg("Failed to notify assigned users after translation submission")
			errList = append(errList, err)
		}
	}

	if len(errList) > 0 {
		return fmt.Errorf("SendToTranslation encountered %d error(s)", len(errList))
	}
	return nil
}
...
func (c *Client) ProcessWebhook(ctx context.Context, originalRequest *http.Request, hookData []byte) (*translations.TranslatableCollection, []common.TranslationData, error) {
	...
	if err != nil {
		log.Error().Err(err).Str("data", string(hookData)).Msg("Unable to unmarshal webhook data")
		return nil, nil, err
	}
	...
	if projectToCheck != c.ProjectUID {
		log.Debug().Str("projectToCheck", projectToCheck).Str("c.ProjectUID", c.ProjectUID).Msg("Ignoring webhook")
		return nil, nil, nil
	}
	...
	if payload.AsyncRequest != nil {
		...
		log.Debug().Str("filename", payload.JobParts[0].FileName).Msg("Unmarshalled json")
		...
		log.Debug().Str("collection", collection.Value).Int("count", len(outData)).Msg("Prepared translations to insert")
	...
	for _, part := range payload.JobParts {
		...
		if part.WorkflowLevel != 3 {
			log.Debug().Str("filename", part.FileName).Int("workflowLevel", part.WorkflowLevel).Msg("Skipping file")
			continue
		}
		...
		if collection == nil {
			// Unknown collection
			log.Warn().Str("filename", part.FileName).Msg("Unknown collection")
			continue
		}
...
func (c *Client) Authenticate() error {
	...
	if res.StatusCode() != 200 {
		return fmt.Errorf("unexpected status code when authenticating to Phrase: %d", res.StatusCode())
	}
...
func (c *Client) GetProject(projectID string) (*Project, error) {
	...
	if res.StatusCode() != 200 {
		log.Error().Bytes("body", res.Body()).Str("projectID", projectID).Int("status", res.StatusCode()).Msg("Unexpected status code when fetching project")
...
func (c *Client) updateJobsStatus(jobs []JobOnlyUID, status Status) error {
	...
	if res.StatusCode() != 200 {
		...
		return fmt.Errorf("unable to update jobs")
	}
...
func (c *Client) CreateJob(targetLanguages []string, path, filename string, data []byte) error {
	...
	if res.StatusCode() != 201 {
		log.Error().Str("projectID", c.ProjectUID).Str("filename", string(res.Body())).Int("status", res.StatusCode()).Msg("Unexpected status code when creating job")
...
func (c *Client) GetJob(jobUID string) (*Job, error) {
	...
	if res.StatusCode() != 200 {
		log.Error().Str("jobUID", jobUID).Int("status", res.StatusCode()).Msg("Unexpected status code when fetching job")
		return nil, err
	}
...
func (c *Client) GetFileAsync(ctx context.Context, jobUID string) error {
	...
	log.Debug().Str("jobUID", jobUID).Str("body", string(res.Body())).Msg("Requested result file")
	...
	// Store async job mapping in PostgreSQL instead of Redis
	err = c.queries.CreatePhraseAsyncJob(ctx, sqlc.CreatePhraseAsyncJobParams{
		AsyncRequestID: webhookID,
		JobUid:         requestID,
	})

	if err != nil {
		log.Debug().Err(err).Msg("Unable to create phrase async job record")
	}
...
func (c *Client) DownloadFile(ctx context.Context, jobUID string, asyncRequestID string) ([]byte, error) {
	...
	// Retrieve async job mapping from PostgreSQL instead of Redis
	downloadID, err := c.queries.GetPhraseAsyncJob(ctx, asyncRequestID)
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, fmt.Errorf("async job not found for request ID: %s", asyncRequestID)
		}
		return nil, err
	}
	...
	req.SetPathParam("projectUid", c.ProjectUID)
	req.SetPathParam("jobUid", jobUID)
	req.SetPathParam("asyncRequestId", downloadID)
	req.SetBody(struct{}{})
	...
	// Clean up the async job record after successful download
	_ = c.queries.DeletePhraseAsyncJob(ctx, asyncRequestID)
...
func (c *Client) NotifyAssignedJobs(projectUID string, jobUIDs []string, emailTemplateID string, cc, bcc []string) error {
	...
	if err != nil {
		log.Error().Str("projectUID", projectUID).Err(err).Msg("Failed to call notifyAssigned")
		return err
	}
